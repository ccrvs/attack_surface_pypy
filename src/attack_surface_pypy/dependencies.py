...
async def get_data_domain(container_: container.CloudSurfaceContainer = fastapi.Depends(get_container)) -> domain.CloudSurfaceDomain:
    return container_.get_data_domain()


async def get_probe_instrumentality(container_: container.CloudSurfaceContainer = fastapi.Depends(get_container)) -> probes.ProbingInstrumentality:
    return container_.get_probe_instrumentality()


async def get_probe(instrumentality: probes.ProbingInstrumentality = fastapi.Depends(get_probe_instrumentality)) -> probes.RouteProbe:
