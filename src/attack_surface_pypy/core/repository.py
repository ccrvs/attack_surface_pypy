...
class CloudDataRepository:
    ...
    def __init__(self, cloud_data: cloud.CloudEnvironmentModel, probe: probes.RepositoryProbe) -> None:
        ...
        self._vm_id_to_vm_map: typing.Optional[
            typing.MutableMapping[types.VM_ID, vm.VMModel]
        ] = None
    ...
    def from_json(
        ...
        warnings.warn("Stale method, use `load_from` instead.",
                      DeprecationWarning, stacklevel=2)  # FIXME: where s my warning?
    ...
    def vm_id_to_vm_map(
        ...
        if self._vm_id_to_vm_map is None:
            self._vm_id_to_vm_map = {
                virtual_machine.vm_id: virtual_machine
                for virtual_machine in self.get_vms()
            }
