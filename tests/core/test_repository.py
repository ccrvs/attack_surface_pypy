...
from attack_surface_pypy.models.v1.models import cloud
...
def loader_mock(cloud_data_mock):
    ...
    loader_mock.load = unittest.mock.Mock(name='load-mock', return_value=cloud_data_mock)
    yield loader_mock


@pytest.fixture
def repository_object(loader_mock):
    yield repository.CloudDataRepository.load_from(loader_mock, unittest.mock.Mock())


def test_load_from_method_actually_calls_load_method_from_passed_loader_object(loader_mock, repository_object):
    assert loader_mock.load.called
    assert isinstance(repository_object, repository.CloudDataRepository)


def test_load_from_method_instantiates_self_with_loaded_data(loader_mock, cloud_data_mock):
    probe_mock = unittest.mock.Mock()
    with unittest.mock.patch.object(repository.CloudDataRepository, '__new__') as new_mock:
        repository.CloudDataRepository.load_from(loader_mock, probe_mock)
    new_mock.assert_called_once_with(repository.CloudDataRepository, cloud_data_mock, probe_mock)


def test_get_vm_by_id_method_return_required_vm_by_its_id_from_prepared_map(loader_mock):
    vm_stub = object()
    with unittest.mock.patch(  # because of __slots__ we can't mock a property of the already created object
        ...
        repository_object = repository.CloudDataRepository.load_from(loader_mock, unittest.mock.Mock())
        assert repository_object.get_vm_by_id(types.VM_ID('some_id')) == vm_stub
        vm_id_map_mock.assert_called_once()


def test_get_vm_by_id_method_on_unknown_key_raises_vm_not_found_error(loader_mock):
    with unittest.mock.patch(  # because of __slots__ we can't mock a property of the already created object
            "attack_surface_pypy.core.repository.CloudDataRepository.vm_id_to_vm_map",
            new_callable=unittest.mock.PropertyMock
    ) as vm_id_map_mock:
        ...
        assert exc_info.value.args[0] == "VM with the given id `any_id` is not found."


def test_get_vms_returns_all_the_vms_from_cloud_data_loaded(repository_object, cloud_data_mock):
    assert repository_object.get_vms() == cloud_data_mock.vms


def test_get_firewall_rules_returns_all_the_fw_rules_from_cloud_data_loaded(repository_object, cloud_data_mock):
    assert repository_object.get_firewall_rules() == cloud_data_mock.fw_rules


def test_vm_id_to_vm_map_gather_its_data_from_the_list_of_vms(repository_object, vms_property_mock):
